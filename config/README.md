...
| File | Role |
|------|------|
| `config.default.json` | Runtime knobs, ports (ignored for 9090–9093 — host contract is in `susi-paths`), routing, `bootstrap_mcp_servers` (small auto-enable set), non-managed `external_peer_agents` (CLI/HTTP/A2A only), governance. **Managed** peers are derived from `execution-agents.json` + `agent-engines.json` `peer_name` fields at load time. |
