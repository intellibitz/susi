...
4. **Pluggable** — Leading catalogs: ≥20 peer agents, ≥20 engines, ≥100 models, ≥1000 MCP scout packages — open-admitted when drivers/keys speak supported protocols (`cli`/`openai_chat`/`http`/`a2a`, OpenAI-compat (+ Anthropic/Gemini/Triton), MCP stdio/HTTP). Local Candle: llama/qwen2 GGUF only.
...
| Plugin protocol agents | **≥20** leading peers in `external_peer_agents` (`cli` / `openai_chat` / `http` / `a2a`); UNAVAILABLE until driver present |
| Plugin protocol engines | **≥20** `inference_endpoints` (+ discovered local ports) |
| Plugin protocol models | **≥100** catalog models (`models.catalog.default.json`) + live `/models` discovery; local GGUF = llama/qwen2 |
| Plugin protocol MCP | **≥1000** scout-registry packages (`mcp.registry.default.json`) provisioned on demand; any extra via `susi mcp-add` |
