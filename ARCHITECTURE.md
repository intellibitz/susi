...
| **Audit** | `susi commits` lists the ledger newest-first with TERM column; `show` dumps a record; `audit` replays and flags signature/gap/duplicate/equivocation/term-regression/non-leader anomalies; `replay` prints the reconstructed `ClusterState`. | `src/cli/commits_cli.rs` |
| **OS view** | `susi os` — one-shot substrate status: consensus term/leader/decisions/anomalies, leaf-service table, verified peer roster. | `src/cli/os_cli.rs` |
