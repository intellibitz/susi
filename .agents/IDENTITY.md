...
**Canonical addressing**: mandates in this document are cited as `Mandate N` (Pillar I) or `Pillar <N> item N` (Pillars II–IV). Source comments predating this document's current structure used to also cite `Rule N`, `Aspiration N`, and `ENGINE-N` — none of those schemes ever corresponded to this document's current numbering, and no reconciliation table exists. As of 2026-09-18 those stale citations were reconciled across all 19 files that carried them (96 occurrences): each was either stripped down to its still-accurate descriptive text (the numbered scheme dropped, the reasoning kept) or, in the one case where the reference was verifiably the same concept under a new name (`admin.rs`'s "Motion Rule"), rewritten to the current `Pillar IV item 3` citation. No new `Rule N`/`Aspiration N`/`ENGINE-N` correspondence to current Mandate/Pillar numbers was invented in the process — where a plausible-looking old number didn't verifiably match a current mandate's substance, the number was dropped rather than guessed. A related, distinct kind of drift is not yet audited: some comments cite `Mandate N` using the *current* terminology with a number that doesn't match this document's actual current mandate at that number (e.g. a comment citing "Mandate 30" for unrelated behavior). That is unscoped, separate follow-up work.
