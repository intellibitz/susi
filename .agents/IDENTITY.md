...
4. **Pluggable** — Curated catalogs: ~10 peer agents, ~15 engines, ~50 models (+ live `/models` discovery), ~100 real MCP packages (+ remote scout) — open-admitted when drivers/keys speak supported protocols (`managed`/`cli`/`openai_chat`/`http`/`a2a`). Top executors lifecycle-managed via `susi agents`. Local Candle: llama/qwen2 GGUF only.
