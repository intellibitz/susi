...
of truth compiled into the binary at build time (see `crates/susi-gawd/src/self_core.rs`).
...
4. **Pluggable** — `CapabilityRegistry` open-admits: OpenAI-compat engines/models (configured URLs + well-known ports; Anthropic/Gemini/Triton via config); any MCP server via `mcp_config.json` / `susi mcp-add`; protocol peers `cli` / `openai_chat` / `http` / `a2a` in `external_peer_agents`. Local Candle: llama/qwen2 GGUF only.
...
23. **Substrate Purity**: Protocol-agnostic admission via registries and config — OpenAI-compat / Anthropic / Gemini / Triton engines, MCP (stdio/HTTP), and peer agents (`cli` / `openai_chat` / `http` / `a2a`). No static domain logic in core; local Candle remains llama/qwen2 GGUF-scoped.
...
36. **Pre-Execution Destructive Pattern Guard**: `SafetyDetector` (`crates/susi-gawd/src/safety.rs`) audits an action's command text against configured destructive-command patterns and critical-system-path patterns before it is allowed to run. This backs Mandate 25 and is what `SafetyAgent` (Pillar II) enforces.
