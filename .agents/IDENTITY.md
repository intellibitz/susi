...
**Canonical addressing**: mandates in this document are cited as `Mandate N` (Pillar I) or `Pillar <N> item N` (Pillars II–IV). Source comments predating this document's current structure also cite `Rule N`, `Aspiration N`, and `ENGINE-N` — none of those schemes correspond to this document's current numbering, and no reconciliation table exists. Treat any `Rule N`/`Aspiration N`/`ENGINE-N` comment as historical context about *why* code was written, not as a live cross-reference into this document; reconciling or removing those stale references across the codebase is tracked as follow-up work, not done in this revision, since it touches many files with case-by-case judgment calls this document alone can't make safely.
