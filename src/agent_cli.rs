...
use susi_agents::external::{catalog, redact, Adapter, AgentManager, RunStatus};
...
pub fn execute(action: AgentCommands, workspace: &Path) -> Result<()> {
    ...
    match action {
        AgentCommands::List => print_json(&catalog()?)?,
        AgentCommands::Setup { agent } => {
            let definition = susi_agents::external::definition(&agent)?;
        ...
        AgentCommands::Doctor { agent } => {
            let agents = match agent {
                Some(id) => vec![susi_agents::external::definition(&id)?],
                None => catalog()?,
            };
