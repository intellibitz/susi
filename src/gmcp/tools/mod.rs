...
impl CoreTools {
    ...
    #[tool(name = "distill_genome", description = "Distill the hard-compiled genome into the Tier 2 reasoning model")]
    ...
    #[tool(name = "self_validate", description = "Execute autonomous substrate self-validation")]
    ...
    #[tool(name = "select_model", description = "Select or override active model substrate")]
    ...
    #[tool(name = "tasks_list", description = "List active and historical swarm tasks with liveness telemetry")]
    ...
    #[tool(name = "tasks_kill", description = "Kill a running or stalled task by task_id")]
    ...
    #[tool(name = "mcp_registry", description = "Interrogate global MCP registry and benchmark servers")]
    ...
    #[tool(name = "agent_register", description = "Dynamically register a new agent profile")]
    ...
    #[tool(name = "power_reason", description = "Delegate complex reasoning to Power-Tier MCP remotes")]
    ...
    #[tool(name = "meta_scout_agents", description = "Discover agent capabilities from connected remotes")]
    ...
    #[tool(name = "meta_rank_agents", description = "Report current agent expertise hierarchy")]
    ...
    #[tool(name = "ast_analyze", description = "Structural AST code analysis via tree-sitter")]
    pub fn ast_analyze(arg: &serde_json::Value, workspace: &Path) -> EaiResult<String> {
        ...
        let code = if let Some(c) = code_s {
            ...
            return Err(EaiError::protocol("Usage: ast_analyze {path: <path>} OR {code: <code>}"));
        };

        let ext = path_s.and_then(|p| Path::new(p).extension()).and_then(|e| e.to_str()).unwrap_or("rs");
        let mut report = format!("AST Analysis ({}): {} bytes\n", ext, code.len());

        // Attempt structural analysis
        if ext == "rs" {
            if let Ok(file) = syn::parse_file(&code) {
                report.push_str(&format!("Converged AST (syn): {} top-level items.\n", file.items.len()));
                for item in file.items.iter().take(5) {
                    match item {
                        syn::Item::Fn(f) => report.push_str(&format!("  - Function: {}\n", f.sig.ident)),
                        syn::Item::Struct(s) => report.push_str(&format!("  - Struct: {}\n", s.ident)),
    ...
    #[tool(name = "semantic_search", description = "Fast embedded search via tantivy")]
    pub fn semantic_search(arg: &serde_json::Value, workspace: &Path) -> EaiResult<String> {
        let query_str = arg.get("query").and_then(|v| v.as_str()).ok_or_else(|| EaiError::protocol("Missing query"))?;
        ...
        let index = Index::open_or_create(tantivy::directory::MmapDirectory::open(&index_path).map_err(|e| EaiError::filesystem(e.to_string()))?, schema.clone())
            .map_err(|e| EaiError::filesystem(e.to_string()))?;

        let reader = index.reader().map_err(|e| EaiError::process(e.to_string()))?;
        let searcher = reader.searcher();

        let query_parser = QueryParser::for_index(&index, vec![content_field]);
        let query = query_parser.parse_query(query_str).map_err(|e| EaiError::process(e.to_string()))?;

        let top_docs = searcher.search(&query, &TopDocs::with_limit(5)).map_err(|e| EaiError::process(e.to_string()))?;
        ...
        for (_score, doc_address) in top_docs {
            let doc: TantivyDocument = searcher.doc(doc_address).map_err(|e| EaiError::process(e.to_string()))?;
            let p = doc.get_first(path_field).and_then(|v| v.as_str()).unwrap_or("unknown");
            out.push_str(&format!("- {}\n", p));
        }
    ...
    #[tool(name = "sandbox_exec", description = "Isolated Docker execution via bollard")]
    pub fn sandbox_exec(arg: &serde_json::Value, _workspace: &Path) -> EaiResult<String> {
        let cmd = arg.get("cmd").and_then(|v| v.as_str()).ok_or_else(|| EaiError::protocol("Missing cmd"))?;
    ...
    #[tool(name = "browser_automate", description = "DOM access and web automation via headless_chrome")]
    pub fn browser_automate(arg: &serde_json::Value, workspace: &Path) -> EaiResult<String> {
        let url = arg.get("url").and_then(|v| v.as_str()).ok_or_else(|| EaiError::protocol("Missing url"))?;

        let browser = Browser::default().map_err(|e| EaiError::process(format!("[CAPABILITY_GAP] Headless Chrome failed: {}. Ensure Chrome/Chromium is installed.", e)))?;
        let tab = browser.new_tab().map_err(|e| EaiError::process(e.to_string()))?;

        tab.navigate_to(url).map_err(|e| EaiError::process(e.to_string()))?;
        tab.wait_until_navigated().map_err(|e| EaiError::process(e.to_string()))?;
        ...
        let content = tab.get_content().map_err(|e| EaiError::process(e.to_string()))?;

        Ok(format!("Browser automation success for {}. Screenshot: {}. Content length: {} bytes.", url, screenshot_path.display(), content.len()))
    }

    #[tool(name = "rag_query", description = "Semantic memory retrieval via Qdrant/FastEmbed")]
    pub fn rag_query(arg: &serde_json::Value, _workspace: &Path) -> EaiResult<String> {
        ...
        rt.block_on(async {
            let client = Qdrant::from_url("http://localhost:6334").build()
                .map_err(|e| EaiError::process(format!("[CAPABILITY_GAP] Qdrant connection failed: {}. Ensure Qdrant is running.", e)))?;

            let search_result = client.search_points(qdrant_client::qdrant::SearchPoints {
                collection_name: "susi_knowledge".to_string(),
                vector,
                limit: 3,
                with_payload: Some(true.into()),
                ..Default::default()
            }).await.map_err(|e| EaiError::process(e.to_string()))?;
    ...
    #[tool(name = "audio_transcribe", description = "Production-grade transcription substrate")]
