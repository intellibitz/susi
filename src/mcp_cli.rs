    ...
    match action.unwrap_or(McpCommands::Serve) {
        ...
        McpCommands::Setup { server } => {
            ...
            print_json(&serde_json::json!({
                ...
                "instructions": "Install Node (npx) and/or uv (uvx). Set any env_keys. Run doctor, then enable to write ~/.susi/mcp_config.json. Override argv/runner with configure. No packages or credentials are provisioned implicitly."
            }))?;
            Ok(false)
        }
