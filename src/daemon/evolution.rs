...
impl EvolutionManager {
    ...
    pub fn perform_autonomous_drift_audit(workspace: &Path) -> EaiResult<String> {
        ...
        if gap != "calculate square root" { // Heuristic check for non-default gap
             ...
             return Ok(format!("Autonomous evolution successful: {}\n\n{}", res, bottlenecks));
        }

        Ok(format!("Substrate Optimal. {}\n\n{}", ingestion_res, bottlenecks))
    }
    ...
    pub fn detect_bottlenecks(workspace: &Path) -> String {
        ...
        for line in log_content.lines() {
            if let Ok(entry) = serde_json::from_str::<serde_json::Value>(line) {
                ...
                } else if event_type == "MISSION_FAILED" || event_type == "TOOL_FAILURE" || details.contains("failed") {
                    mission_failures += 1;
                }
            }
        }
        ...
        let mut report = format!("# Substrate Bottleneck Analysis\n\n\
