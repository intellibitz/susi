...
use susi_agents::external::{
    gemini_cli_doctor, gemini_cli_setup, gemini_cli_status, AgentManager, CatalogKind, RunStatus,
    GEMINI_CLI_AGENT_ID,
};

#[derive(Debug, Subcommand)]
pub enum GeminiCliCommands {
    /// Show Gemini CLI binary + auth status
    List,
    /// Check CLI binary and credentials (does not start a paid run)
    Doctor,
    /// Show install/auth instructions and effective adapter
    Setup,
    /// Start a headless Gemini CLI task (same lifecycle as `susi agents run gemini-cli`)
    Run {
        #[arg(long)]
        wait: bool,
        prompt: String,
    },
    /// List durable Gemini CLI tasks in this workspace
    Tasks,
    Status {
        task_id: String,
        #[arg(long)]
...
pub fn execute(action: Option<GeminiCliCommands>, workspace: &Path) -> Result<()> {
    let action = action.unwrap_or(GeminiCliCommands::List);
    let manager = AgentManager::new(workspace)?;
    match action {
        GeminiCliCommands::List => {
            let def =
                susi_agents::external::definition(CatalogKind::Execution, GEMINI_CLI_AGENT_ID)?;
            print_json(&serde_json::json!({
                "status": gemini_cli_status(),
                "definition": def,
            }))?;
        }
        GeminiCliCommands::Doctor => {
            let detail = gemini_cli_doctor()?;
            print_json(&serde_json::json!({
                "agent": GEMINI_CLI_AGENT_ID,
                "prerequisites_present": true,
                "detail": detail,
            }))?;
        }
        GeminiCliCommands::Setup => print_json(&gemini_cli_setup())?,
        GeminiCliCommands::Run { wait, prompt } => {
            gemini_cli_doctor()?;
            let run = manager.prepare(GEMINI_CLI_AGENT_ID, &prompt)?;
            launch(&manager, &run.id, wait)?;
        }
        GeminiCliCommands::Tasks => {
            let runs: Vec<_> = manager
                .list()?
                .into_iter()
                .filter(|r| r.agent == GEMINI_CLI_AGENT_ID)
                .collect();
            print_json(&runs)?;
        }
        GeminiCliCommands::Status { task_id, refresh } => {
        ...
        GeminiCliCommands::Cancel { task_id } => print_json(&manager.cancel(&task_id)?)?,
    }
    Ok(())
}
...
fn launch(manager: &AgentManager, id: &str, wait: bool) -> Result<()> {
    ...
    if std::env::var_os("SUSI_PROCESS_BANNER").is_none() {
        command.env("SUSI_PROCESS_BANNER", "susi-gemini-cli");
    }
    #[cfg(unix)]
    {
...
fn run_worker(manager: AgentManager, id: &str) -> Result<()> {
    ...
    if matches!(
        ...
        bail!(
            "gemini-cli task {} ended with {:?}; inspect status and logs",
            run.id,
            run.status
        );
    }
