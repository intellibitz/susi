...
use susi_agents::external::{
    aider_doctor, aider_setup, aider_status, AgentManager, CatalogKind, RunStatus, AIDER_AGENT_ID,
};

#[derive(Debug, Subcommand)]
pub enum AiderCommands {
    /// Show Aider binary + LLM env status
    List,
    /// Check CLI binary and provider credentials (does not start a paid run)
    Doctor,
    /// Show install/auth instructions and effective adapter
    Setup,
    /// Start a headless Aider task (same lifecycle as `susi agents run aider`)
    Run {
        #[arg(long)]
        wait: bool,
        prompt: String,
    },
    /// List durable Aider tasks in this workspace
    Tasks,
    Status {
        task_id: String,
        #[arg(long)]
...
pub fn execute(action: Option<AiderCommands>, workspace: &Path) -> Result<()> {
    let action = action.unwrap_or(AiderCommands::List);
    let manager = AgentManager::new(workspace)?;
    match action {
        AiderCommands::List => {
            let def = susi_agents::external::definition(CatalogKind::Execution, AIDER_AGENT_ID)?;
            print_json(&serde_json::json!({
                "status": aider_status(),
                "definition": def,
            }))?;
        }
        AiderCommands::Doctor => {
            let detail = aider_doctor()?;
            print_json(&serde_json::json!({
                "agent": AIDER_AGENT_ID,
                "prerequisites_present": true,
                "detail": detail,
            }))?;
        }
        AiderCommands::Setup => print_json(&aider_setup())?,
        AiderCommands::Run { wait, prompt } => {
            aider_doctor()?;
            let run = manager.prepare(AIDER_AGENT_ID, &prompt)?;
        ...
        AiderCommands::Tasks => {
            let runs: Vec<_> = manager
                .list()?
                .into_iter()
                .filter(|r| r.agent == AIDER_AGENT_ID)
                .collect();
            print_json(&runs)?;
        }
        AiderCommands::Status { task_id, refresh } => {
        ...
        AiderCommands::Cancel { task_id } => print_json(&manager.cancel(&task_id)?)?,
    }
    Ok(())
}
...
fn launch(manager: &AgentManager, id: &str, wait: bool) -> Result<()> {
    ...
    if std::env::var_os("SUSI_PROCESS_BANNER").is_none() {
        command.env("SUSI_PROCESS_BANNER", "susi-aider");
    }
    #[cfg(unix)]
    {
...
fn run_worker(manager: AgentManager, id: &str) -> Result<()> {
    ...
    if matches!(
        ...
        bail!(
            "aider task {} ended with {:?}; inspect status and logs",
            run.id,
            run.status
        );
    }
