...
// or the API's own authoritative token count; where a genuine count isn't
...
use crate::gemi::models::ModelBenchmarkResult;
use std::path::Path;
...
impl BenchmarkRunner {
    ...
    /// ids aren't exposed).
    pub fn benchmark_local(prompt: &str, workspace: &Path) -> EaiResult<ModelBenchmarkResult> {
        let model_id = crate::gemi::models::ModelManager::get_selected_model(Some(
            crate::gemi::intent::IntentClassifier::classify(prompt),
        ))
        .ok_or_else(|| EaiError::inference("No local reasoning model selected."))?;

        let start = Instant::now();
        let output = crate::gemi::engine::GemiEngine::generate_reasoning_deep(prompt, workspace);
        let elapsed = start.elapsed();

        let token_count = crate::gemi::models::ModelManager::get_tokenizer_path(&model_id)
    ...
    /// vocabulary; falls back to a whitespace-count estimate — reflected in
    /// `status`, never silently presented as the authoritative count — only
    /// when the API response omits usage entirely.
    ///
    ...
    pub fn benchmark_cloud(prompt: &str) -> EaiResult<Option<ModelBenchmarkResult>> {
        ...
        let start = Instant::now();
        let mut req = crate::sandbox::manager::http_agent()
            .post(&url)
        ...
        let (token_count, status) = match body["usage"]["completion_tokens"].as_u64() {
            Some(n) => (n as usize, "OK".to_string()),
            None if !completion_text.trim().is_empty() => (
                completion_text.split_whitespace().count(),
                "OK (token count estimated: API returned no usage field)".to_string(),
            ),
            None => (0, "EMPTY_OUTPUT".to_string()),
        };
    ...
    pub fn run_and_render(workspace: &Path) -> String {
        ...
        if cloud_results.is_empty() {
            out.push_str(
                "No cloud comparison run (set SUSI_BENCH_CLOUD_API_BASE to enable one).\n",
            );
        } else {
            out.push_str(&format!(
