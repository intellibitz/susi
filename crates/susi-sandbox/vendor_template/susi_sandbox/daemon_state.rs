...
impl SusiDaemonState {
    ...
    pub fn calculate_binary_hash_cached(
        ...
        if let Some(hash) = crate::susi_sandbox::service::hash_cached(bin_path, global_dir) {
            return Ok(hash);
        }
    ...
    pub fn verify_binary_integrity(bin_path: &Path, global_dir: &Path) -> std::io::Result<bool> {
        if let Some(ok) = crate::susi_sandbox::service::verify_integrity(bin_path, global_dir) {
            return Ok(ok);
        }
    ...
    pub fn check_status(workspace: &Path, global_dir: &Path) -> bool {
        if let Some(status) = crate::susi_sandbox::service::check_status(workspace, global_dir) {
            return status;
        }
