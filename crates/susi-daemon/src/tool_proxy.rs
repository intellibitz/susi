    ...
    pub fn execute(&self, command: &str, args: &[String]) -> Result<Output, ToolError> {
        ...
        if authorized {
            ...
            if command.contains("..") || command.contains('/') || command.contains('\\') {
                return Err(ToolError::InvalidCommand("Path traversal or absolute paths denied".into()));
            }
...
mod tests {
    ...
    fn test_tool_proxy_enforcement() {
        ...
        // Granted policy
        let granted_policy = CapabilityPolicy::new("test-cell", vec![
            CapabilityGrant {
        ...
        let escape_policy = CapabilityPolicy::new("test-cell", vec![
            CapabilityGrant {
