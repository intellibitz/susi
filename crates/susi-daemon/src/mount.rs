...
impl MountManager {
    ...
    pub fn mount_dir(&self, policy: &CapabilityPolicy, cell_path: &str, host_path: &Path, read_only: bool) -> Result<(), String> {
        // Verify Capability
        ...
        if !has_mount {
            return Err(format!("Access Denied: Cell '{}' lacks the 'mount' capability.", policy.cell_id()));
        }
    ...
    pub fn resolve_path(&self, cell_id: &str, requested_cell_path: &str) -> Result<PathBuf, String> {
...
mod tests {
    ...
    fn test_mount_capability_and_resolution() {
        ...
        let privileged_policy = CapabilityPolicy::new("cell-priv", vec![
            CapabilityGrant { capability: "mount".to_string(), scope: None, ephemeral: false }
        ]);
        
        // Standard cell should fail
        assert!(manager.mount_dir(&standard_policy, "/work", Path::new("/home/user"), true).is_err());
        
        // Privileged cell should succeed
        assert!(manager.mount_dir(&privileged_policy, "/work", Path::new("/home/user"), false).is_ok());
        ...
        // Unknown path
        assert!(manager.resolve_path("cell-priv", "/other/data.txt").is_err());
    }
}
