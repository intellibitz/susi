...
/// Owner-side dispatch: MAC + evidence capture around the real tool, so the
/// execution boundary is enforced inside the copy that owns the capability.
struct ToolDispatch {
    tool: Arc<dyn Tool>,
}
...
impl PlaneHandler for ToolDispatch {
    fn handle(&self, _topic: &str, payload: Value) -> Result<Value, String> {
        ...
        let ws = payload
            ...
            .to_string();
        let ws = Path::new(&ws);
        let res = MacPolicy::global()
            .authorize_tool(self.tool.name(), &args, ws, None)
            .and_then(|()| {
                EvidenceSession::capture_call(self.tool.name(), &args, ws, || {
                    self.tool.execute(&args, ws)
                })
            });
        res.map(|out| json!({ "output": out }))
...
impl IpcCapabilityRegistry {
    ...
    /// Register a tool: kept in the local map for the fast path, served to
    /// other copies through `capability.tool.<name>` on the bus.
